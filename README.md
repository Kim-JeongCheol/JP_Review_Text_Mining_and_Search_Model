# JobPlanet Review Text Mining and Search Model
잡플래닛 리뷰 기반 텍스트 마이닝 & 검색 모델 프로젝트 (SAPI 팀)
...
| 지표 | 설명 | 값 |
|------|------|----|
| **평균 코사인 유사도**<br>(Mean Cosine Similarity) | 여러 문장 간 코사인 유사도를 계산한 뒤 평균낸 값이다.<br>텍스트 전체의 의미적 일관성과 문맥적 유사성을 종합적으로 나타낸다. | **0.34** |

#### **종합 해석**  

<img width="608" height="371" alt="image" src="https://github.com/user-attachments/assets/c959a0b5-fb14-4096-8d52-1a67b3bfe93f" />  

‘복지(0.372)’와 ‘소통(0.383)’ 키워드의 평균 유사도가 상대적으로 높아,  
리뷰 내에서 해당 주제가 가장 일관된 문맥으로 언급되고 있음을 확인할 수 있다.  

반면 ‘야근(0.315)’과 ‘승진(0.314)’ 키워드는 유사도가 낮아,  
리뷰 간 표현 다양성이 크거나 언급 빈도가 낮은 경향을 보인다.  

전반적으로 **평균 유사도는 0.31~0.38 수준**으로,  
리뷰 간 **의미적 일관성이 중간 이상 수준**으로 유지되고 있다.  
이는 **TF-IDF 기반 코사인 벡터화 모델**이 안정적인 **의미 탐색 성능**을 보이며,  
리뷰 데이터가 ‘복지’, ‘소통’ 등 **핵심 주제를 중심으로 일관된 서술 패턴을 형성**하고 있음을 시사한다.
---
...
> **작성자**: Kim Jeong Cheol (팀장 / 데이터 수집·전처리·EDA·가설 검정·모델링 총괄)  
> **소속 팀**: SAPI (사피)  
